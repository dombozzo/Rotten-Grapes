...
class _wine_database:
    ...
    def load_all(self, wine_file):
        ...
        for index, row in df.iterrows():
            ...
            if row['variety_id'] not in self.vars:
                self.vars[row['variety_id']] = row['variety']
    ...
    def get_variety(self, wid):
        ...
        try:
            ...
            for index, row in self.reviews.iterrows():
                if str(row['bottle_id']) == str(wid):
                    return row['variety_id']
        except:
            return None
    ...
    def set_review(self, uid, wid, review_info):
        ...
        new_entry = {}
        try:
            user_info = self.users[int(uid)]
            wine_info = self.wines[int(wid)]
        except:
            return None
